...
pub fn patch_emit_function(ctx: &mut EmitContext, f: &Function) -> Result<bool, EmitErr> {
    if let Some(_guard) = ctx.disable_patch_guard_if_patch_enabled() {
        ...
        return match (module.as_str(), f.ident.as_str()) {
            (_, "__debugbreak" | "_ReadWriteBarrier") => Ok(true), // skip emit
            ("atomic", "SDL_MemoryBarrierAcquireFunction" | "SDL_MemoryBarrierReleaseFunction") => {
            ...
            ("bits", "SDL_MostSignificantBitIndex32") => {
                f.doc.emit(ctx)?;
            ...
            ("stdinc", "SDL_copysign" | "SDL_copysignf") => {
                let mut fr = f.clone();
                let arg0 = &f.args.args[0];
            ...
            ("vulkan", "SDL_Vulkan_GetVkGetInstanceProcAddr") => {
                let mut f = f.clone();
...
pub fn patch_emit_define(ctx: &mut EmitContext, define: &Define) -> Result<bool, EmitErr> {
    if let Some(_guard) = ctx.disable_patch_guard_if_patch_enabled() {
        let module = ctx.module().to_owned();
        return match (module.as_str(), define.ident.as_str()) {
            (
                _,
                "main"
                | "SDL_arraysize"
            ...
            ("atomic", "SDL_CPUPauseInstruction") => {
                define.doc.emit(ctx)?;
                // TODO: add more archs
                ctx.write_str(str_block! {r#"
                    #[inline(always)]
            ...
            ("stdinc", i) if i.starts_with("SDL_PRI") => {
            ...
            ("thread", "SDL_CreateThread" | "SDL_CreateThreadWithProperties") => {
...
fn emit_begin_end_thread_function(ctx: &mut EmitContext) -> EmitResult {
    ...
    writeln!(ctx, "{cfg_win}")?;
    writeln!(ctx, "unsafe extern \"C\" {{")?;
    ctx.increase_indent();
    writeln!(
        ctx,
        "fn _beginthreadex(security: *mut ::core::ffi::c_void, stack_size: ::core::ffi::c_uint, start_address: Option<unsafe extern \"system\" fn(*const ::core::ffi::c_void) -> ::core::ffi::c_uint>, arglist: *mut ::core::ffi::c_void, initflag: ::core::ffi::c_uint, thrdaddr: ::core::ffi::c_uint) -> ::core::primitive::usize;"
    )?;
    ...
    writeln!(
        ctx,
        "pub const SDL_BeginThreadFunction: SDL_FunctionPointer = unsafe {{ ::core::mem::transmute::<unsafe extern \"C\" fn(*mut ::core::ffi::c_void, ::core::ffi::c_uint, Option<unsafe extern \"system\" fn(*const ::core::ffi::c_void) -> ::core::ffi::c_uint>, *mut ::core::ffi::c_void, ::core::ffi::c_uint, ::core::ffi::c_uint) -> ::core::primitive::usize, SDL_FunctionPointer>(_beginthreadex) }};"
    )?;
    writeln!(ctx, "{cfg_win}")?;
    writeln!(
        ctx,
        "pub const SDL_EndThreadFunction: SDL_FunctionPointer = unsafe {{ ::core::mem::transmute::<unsafe extern \"C\" fn (::core::ffi::c_uint), SDL_FunctionPointer>(_endthreadex) }};"
    )?;
    writeln!(ctx)?;
    Ok(())
}
...
pub fn patch_emit_macro_call(
    ...
    if let Some(_guard) = ctx.disable_patch_guard_if_patch_enabled() {
        let module = ctx.module().to_owned();
        return match (module.as_str(), ident) {
            (_, "SDL_COMPILE_TIME_ASSERT") => {
                write!(ctx, "const _: () = ")?;
...
pub fn patch_eval_macro_call(
    ...
    match ident {
        "defined" if ctx.is_preproc_eval_mode() => {
            let args = &*call.args;
            let err = || {
                Err(ParseErr::new(
                    call.span(),
...
pub fn patch_emit_struct_or_union(
    ...
    if let Some(_guard) = ctx.disable_patch_guard_if_patch_enabled() {
        let module = ctx.module().to_owned();
        return match (module.as_str(), ident) {
            ("textengine", "TTF_TextData" | "TTF_TextEngine") => {
                TypeDef {
                    span: Span::none(),
                    doc: None,
...
pub fn patch_emit_type_def(
    ...
    if let Some(_guard) = ctx.disable_patch_guard_if_patch_enabled() {
        ...
        return match (module.as_str(), ident) {
            ...
            ("ttf", "TTF_TextData" | "TTF_TextEngine") => {
                td.doc.emit(ctx)?;
